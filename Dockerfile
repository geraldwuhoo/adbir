...
# chef
FROM docker.io/library/rust:1.98.1 AS chef
...
# get dashboard-icons
FROM docker.io/curlimages/curl:8.21.0 AS dashboard-icons
...
# dummy icons (none)
FROM docker.io/library/alpine:3.24.1 AS no-icons
...
RUN apk add -U --no-cache darkhttpd=1.17-r0
